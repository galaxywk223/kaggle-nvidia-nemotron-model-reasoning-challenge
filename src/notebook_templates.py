...
REMOTE_COPY_SUBMIT_TEMPLATE = """from __future__ import annotations
...
def zip_priority(path: Path) -> tuple[int, int, str]:
    ...
    if name == "submission.zip":
        ...
        name_rank = 2
    return (name_rank, len(str(path)), str(path))


def find_valid_source_zip() -> Path | None:
    candidates = sorted(Path("/kaggle/input").rglob("*.zip"), key=zip_priority)
