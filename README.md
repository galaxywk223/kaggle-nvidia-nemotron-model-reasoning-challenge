...
LoRA SFT converts `prompt`, `answer`, and `generated_cot` fields into chat-style training examples. The user side appends a requirement that the final answer appear in `\boxed{}`. The assistant side keeps the generated reasoning trace, removes any existing boxed answer from the raw CoT, and appends a normalized `</think>` plus boxed target answer. Training uses response-only labels derived from tokenizer offset mappings, so prompt and chat-template prefix tokens are masked out. The final `</think>` and `\boxed{answer}` suffix receives 2x loss weight to strengthen the answer format and final-answer tokens used by the competition metric.
